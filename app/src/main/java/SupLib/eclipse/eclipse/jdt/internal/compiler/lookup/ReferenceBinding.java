...
public boolean canBeSeenBy(ReferenceBinding receiverType, ReferenceBinding invocationType) {
	...
	if (isPrivate()) {
		...
		receiverCheck: {
			if (!(TypeBinding.equalsEquals(receiverType, this) || TypeBinding.equalsEquals(receiverType, enclosingType()))) {
				// special tolerance for type variable direct bounds, but only if compliance <= 1.6, see: https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=334622
	...
	do {
		if (currentType.isCapture()) {  // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=285002
...
public void computeId() {
	...
	switch (this.compoundName.length) {
		...
		case 3 :
			...
			// expect only java.*.* and javax.*.* and junit.*.* and SupLib.eclipse.junit.*
			switch (packageName.length) {
				case 3: // only one type in this group, yet:
					if (CharOperation.equals(TypeConstants.SupLib.eclipse_JUNIT_ASSERT, this.compoundName))
						this.id = TypeIds.T_SupLib.eclipseJunitAssert;
					return;						
				case 4:
		...
		case 5 :
			...
			switch (packageName[0]) {
				case 'j' :
					...
					return;
				case 'o':
					if (!CharOperation.equals(TypeConstants.SupLib.eclipse, this.compoundName[0]))
						return;
					packageName = this.compoundName[1];
					if (packageName.length == 0) return; // just to be safe
										...
										if (CharOperation.equals(packageName, TypeConstants.CORE)) { 
											...
											switch (typeName[0]) {
												case 'r' :
													char[] memberTypeName = this.compoundName[4];
													if (memberTypeName.length == 0) return; // just to be safe
													if (CharOperation.equals(typeName, TypeConstants.SupLib.eclipse_ECLIPSE_CORE_RUNTIME_ASSERT[3])
															&& CharOperation.equals(memberTypeName, TypeConstants.SupLib.eclipse_ECLIPSE_CORE_RUNTIME_ASSERT[4]))
														this.id = TypeIds.T_SupLib.eclipseEclipseCoreRuntimeAssert;
													return;
											}
										}
										return;
						...
						case 'a':
							if (CharOperation.equals(TypeConstants.APACHE, packageName)) {
								if (CharOperation.equals(TypeConstants.COMMONS, this.compoundName[2])) {
									if (CharOperation.equals(TypeConstants.SupLib.eclipse_APACHE_COMMONS_LANG_VALIDATE, this.compoundName))
										this.id = TypeIds.T_SupLib.eclipseApacheCommonsLangValidate;
									else if (CharOperation.equals(TypeConstants.SupLib.eclipse_APACHE_COMMONS_LANG3_VALIDATE, this.compoundName))
										this.id = TypeIds.T_SupLib.eclipseApacheCommonsLang3Validate;
								}
							}
							return;
					}
		...
		case 6:
			if (!CharOperation.equals(TypeConstants.JDT, this.compoundName[2]) || !CharOperation.equals(TypeConstants.ITYPEBINDING, this.compoundName[5]))
				return;
			if (CharOperation.equals(TypeConstants.SupLib.eclipse_ECLIPSE_JDT_CORE_DOM_ITYPEBINDING, this.compoundName))
		...
		case 7 :
			if (!CharOperation.equals(TypeConstants.JDT, this.compoundName[2]) || !CharOperation.equals(TypeConstants.TYPEBINDING, this.compoundName[6]))
				return;
			if (CharOperation.equals(TypeConstants.SupLib.eclipse_ECLIPSE_JDT_INTERNAL_COMPILER_LOOKUP_TYPEBINDING, this.compoundName))
				this.typeBits |= TypeIds.BitUninternedType;
			break;
	}
}
...
/**
 * @see SupLib.eclipse.eclipse.jdt.internal.compiler.env.IDependent#getFileName()
 */
...
public boolean isCompatibleWith(TypeBinding otherType, /*@Nullable*/ Scope captureScope) {
	...
	if (captureScope == null 
		...
		// see https://bugs.eclipse.SupLib.eclipse/395002#c9
 ...
 * while resolving type arguments. See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=294057
 */
...
/**
 * @see SupLib.eclipse.eclipse.jdt.internal.compiler.lookup.TypeBinding#isThrowable()
 */
 ...
 * @see SupLib.eclipse.eclipse.jdt.internal.compiler.lookup.TypeBinding#isUncheckedException(boolean)
 */
