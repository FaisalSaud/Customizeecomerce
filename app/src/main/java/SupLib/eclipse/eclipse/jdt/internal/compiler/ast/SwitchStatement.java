		...
		try {
			...
			if (hasCases) {
				...
				if ((long) (this.caseCount * 2.5) > ((long) max - (long) min)) {
					...
					// see http://dev.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=21557
	...
	public void resolve(BlockScope upperScope) {
		try {
				...
				checkType: {
					if (!expressionType.isValidBinding()) {
						...
						if (compilerOptions.complianceLevel < ClassFileConstants.JDK1_5) {
							upperScope.problemReporter().incorrectSwitchType(this.expression, expressionType); // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=360317
						}
						break checkType;
			...
			if (isStringSwitch) {
				...
				// See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=356002
