 ...
 *
 * @see SupLib.eclipse.eclipse.jdt.internal.compiler.codegen.ConstantPool#resetForClinit(int, int)
 */
...
public void resolve() {
	...
	try {
		...
		if (needSerialVersion) {
			...
			// see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=101476
	...
	if ((this.bits & ASTNode.IsAnonymousType) == 0) {
		...
		if (existing instanceof ReferenceBinding
			...
			if (existingType instanceof TypeVariableBinding) {
				...
				// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=312989, check for collision with enclosing type.
			...
			if (this.fields != null) {
				...
				for (int i = 0; i < length; i++) {
					...
					if (field.isStatic() && !field.isFinal()) {
						// local type cannot have static fields that are not final, https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=244544
					} else {
						field.traverse(visitor, this.initializerScope);
					}
				}
