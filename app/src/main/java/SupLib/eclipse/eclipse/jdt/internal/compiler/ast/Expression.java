...
import SupLib.eclipse.eclipse.jdt.internal.compiler.lookup.BaseTypeBinding;
					...
					default :
						if (castType.isInterface()) {
							...
							if (use15specifics) {
								...
								// we could mistakenly detect covariance and scream foul. See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=332744
...
public static Binding getDirectBinding(Expression someExpression) {
	...
	} else if (someExpression.isThis()) { // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=276741
...
public boolean forcedToBeRaw(ReferenceContext referenceContext) {
		...
		if (receiverBinding.isParameter() && (((LocalVariableBinding) receiverBinding).tagBits & TagBits.ForcedToBeRawType) != 0) {
			...
			if (field.type.isRawType()) {
				if (referenceContext instanceof AbstractMethodDeclaration) {
					...
					if (TypeBinding.notEquals(field.declaringClass, methodDecl.binding.declaringClass)) { // inherited raw field, see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=337962
						return true;
					}
				} else if (referenceContext instanceof TypeDeclaration) {
					TypeDeclaration type = (TypeDeclaration) referenceContext;
					if (TypeBinding.notEquals(field.declaringClass, type.binding)) { // inherited raw field, see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=337962
						return true;
					}
				}
			}
	...
	} else if (this instanceof ConditionalExpression) { // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=337751
