...
public boolean isProvablyDistinct(TypeBinding otherType) {
	...
	/* With the hybrid 1.4/1.5+ projects modes, while establishing type equivalence, we need to
	   ...
	   not. See:
	    https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=186565
        https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=328827
        https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=329588
	 */ 
    ...
    switch (kind()) {
		...
		case Binding.PARAMETERIZED_TYPE :
		    ...
		    switch(otherType.kind()) {
		            ...
		            if (!paramType.isStatic()) { // static member types do not compare their enclosing
		            	...
		            	if (enclosing != null) {
		            		...
		            		if ((otherEnclosing.tagBits & TagBits.HasDirectWildcard) == 0) {
		            			if (enclosing.isProvablyDistinct(otherEnclosing)) return true; // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=302919
		            		} else {
		    	...
		    	case Binding.TYPE:  // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=329588
...
public boolean isTypeArgumentContainedBy(TypeBinding otherType) {
	...
	switch (otherType.kind()) {
		...
		case Binding.TYPE_PARAMETER: {
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=347426
			...
			if (upperBound.id == TypeIds.T_JavaLangObject && otherBounds == null) {
				return false; // but given class A<T>, A<?> stays an unbounded wildcard, see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=348956
			}
