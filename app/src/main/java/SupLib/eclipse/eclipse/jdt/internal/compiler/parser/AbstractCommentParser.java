					...
					case '@' :
						...
						if ((!this.lineStarted || previousChar == '{')) {
							if (this.inlineTagStarted) {
								setInlineTagStarted(false);
								// bug https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=53279
	...
	protected boolean parseReference() throws InvalidInputException {
		...
		try {
					...
					case TerminalTokens.TokenNameERROR :
						...
						if (currentError.length>0 && currentError[0] == '"') {
							if (this.reportProblems) {
								...
								if (isUrlRef) {
									// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=207765
			...
			switch (ch) {
				...
				// See bug https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=47215
				case '(' :
				...
				// See bug https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=168849
				case ':' :
					ch = readChar();
					if (ch == '/' && ch == readChar()) {
						if (this.reportProblems) {
