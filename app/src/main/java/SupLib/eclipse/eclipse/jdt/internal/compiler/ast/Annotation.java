...
public abstract class Annotation extends Expression {
	...
	public boolean isRuntimeInvisible() {
		...
		// we need to filter out only "pure" type use and type parameter annotations, see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=392119
...
//		if (scope.compilerOptions().storeAnnotations)
	...
	static boolean isAnnotationTargetAllowed(Annotation annotation, BlockScope scope, TypeBinding annotationType, int kind) {
		...
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=391201
		...
		switch (kind) {
			...
			case Binding.TYPE_PARAMETER : // jsr308
				// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=391196
				if ((metaTagBits & (TagBits.AnnotationForTypeParameter | TagBits.AnnotationForTypeUse)) != 0) {
					return true;
				}
		}
