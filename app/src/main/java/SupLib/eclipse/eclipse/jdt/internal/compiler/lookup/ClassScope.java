...
public class ClassScope extends Scope {
	...
	void buildFields() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (field.getKind() == AbstractVariableDeclaration.INITIALIZER) {
				...
				// now this error reporting is moved into the parser itself. See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=212713
			} else {
	...
	void buildMethods() {
		...
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=243917, conservatively tag all methods and fields as
		...
		if (hasNativeMethods) {
			...
			FieldBinding[] fields = sourceType.unResolvedFields(); // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=301683
...
//					realModifiers = modifiers & ExtraCompilerModifiers.AccJustFlag;
	...
	*/
	private void checkAndSetModifiersForField(FieldBinding fieldBinding, FieldDeclaration fieldDecl) {
		...
		if (declaringClass.isInterface()) {
			...
			// set the modifiers
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=267670. Force all enumerators to be marked
	...
	*/
	...
	public boolean detectHierarchyCycle(TypeBinding superType, TypeReference reference) {
		...
		// Reinstate the code deleted by the fix for https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=205235
		// For details, see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=294057.
	...
	private boolean detectHierarchyCycle(SourceTypeBinding sourceType, ReferenceBinding superType, TypeReference reference) {
		...
		if (superType.isHierarchyBeingActivelyConnected()) {
			SupLib.eclipse.eclipse.jdt.internal.compiler.ast.TypeReference ref = ((SourceTypeBinding) superType).scope.superTypeReference;
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=133071
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=121734
			...
			if (ref != null && ref.resolvedType == null) {
				// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=319885 Don't cry foul prematurely.
