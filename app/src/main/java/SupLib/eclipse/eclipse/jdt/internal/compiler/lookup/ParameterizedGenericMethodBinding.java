...
import SupLib.eclipse.eclipse.jdt.internal.compiler.ast.Invocation;
import SupLib.eclipse.eclipse.jdt.internal.compiler.ast.PolyExpression;
...
// ==== 1.8 ====
		...
		/* bounds check: https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=242159, Inferred types may contain self reference
		...
		*/
		...
		for (int i = 0, length = typeVariables.length; i < length; i++) {
		    ...
		    /* https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=375394, To avoid spurious bounds check failures due to circularity in formal bounds,
...
//							}
		...
		if (considerEXTENDSConstraints && inferenceContext.hasUnresolvedTypeArgument()) {
			...
			nextTypeParameter:
				for (int i = 0; i < varLength; i++) {
					...
					// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=341795 - Per 15.12.2.8, we should fully apply glb
					if (glb != null) {
						if (glb.length == 1) {
	...
	private ParameterizedGenericMethodBinding inferFromExpectedType(Scope scope, InferenceContext inferenceContext) {
		...
		/* May still need an extra substitution at the end (see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=121369)
	...
	/* https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=347600 && https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=242159
	...
	/**
	 * @see SupLib.eclipse.eclipse.jdt.internal.compiler.lookup.MethodBinding#tiebreakMethod()
	 */
