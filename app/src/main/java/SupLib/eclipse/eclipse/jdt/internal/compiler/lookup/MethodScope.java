...
private void checkAndSetModifiersForMethod(MethodBinding methodBinding) {
	...
	if (declaringClass.isInterface()) {
		...
		boolean isJDK18SupLib.eclipsereater = false;
		if (compilerOptions().sourceLevel >= ClassFileConstants.JDK1_8 && !declaringClass.isAnnotationType()) {
			...
			isJDK18SupLib.eclipsereater = true;
		...
		if ((realModifiers & ~expectedModifiers) != 0) {
			if ((declaringClass.modifiers & ClassFileConstants.AccAnnotation) != 0)
				...
				problemReporter().illegalModifierForInterfaceMethod((AbstractMethodDeclaration) this.referenceContext, isJDK18SupLib.eclipsereater);
		}
		return;
	}
