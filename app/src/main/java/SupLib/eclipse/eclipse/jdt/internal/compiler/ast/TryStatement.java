...
private MethodBinding findCloseMethod(final LocalDeclaration resource, LocalVariableBinding resourceBinding) {
	...
	if (type != null && type.isValidBinding()) {
		...
		if(closeMethod == null) {
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=380112
...
private FlowInfo prepareCatchInfo(FlowInfo flowInfo, ExceptionHandlingFlowContext handlingContext, FlowInfo tryInfo, int i) {
	...
	*/
	if (this.tryBlock.statements == null && this.resources == NO_RESOURCES) { // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=350579
...
public void generateCode(BlockScope currentScope, CodeStream codeStream) {
	...
	try {
		...
		if (resourceCount > 0) {
			// Please see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=338402#c16
		...
		if (resourceCount > 0) {
			for (int i = resourceCount; i >= 0; i--) {
				...
				if ((this.bits & ASTNode.IsTryBlockExiting) == 0) {
					...
					if (i > 0) {
						int invokeCloseStartPc = codeStream.position; // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=343785
						if (this.postTryInitStateIndex != -1) {
							/* https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=361053, we are just past a synthetic instance of try-catch-finally.
		...
		this.resourceExceptionLabels = null;  // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=375248
	}
...
public boolean generateSubRoutineInvocation(BlockScope currentScope, CodeStream codeStream, Object targetLocation, int stateIndex, LocalVariableBinding secretLocal) {
	...
	int resourceCount = this.resources.length;
	if (resourceCount > 0 && this.resourceExceptionLabels != null) { // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=375248
		for (int i = resourceCount; i > 0; --i) {
...
public void resolve(BlockScope upperScope) {
	...
	for (int i = 0; i < resourceCount; i++) {
		...
		if (localVariableBinding != null && localVariableBinding.isValidBinding()) {
			...
			} else if (resourceType != null) { // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=349862, avoid secondary error in problematic null case
				upperScope.problemReporter().resourceHasToImplementAutoCloseable(resourceType, this.resources[i].type);
				localVariableBinding.type = new ProblemReferenceBinding(CharOperation.splitOn('.', resourceType.shortReadableName()), null, ProblemReasons.InvalidTypeForAutoManagedResource);
			}
		}
