...
public MethodBinding asRawMethod(LookupEnvironment env) {
	...
	for (int i = 0; i < length; i++) {
		...
		if (var.boundsCount() <= 1) {
			...
			arguments[i] = env.createWildcard(null, 0, rawFirstBound, rawOtherBounds, SupLib.eclipse.eclipse.jdt.internal.compiler.ast.Wildcard.EXTENDS);
		}
	}
 ...
 *
 * See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=171184
 */
