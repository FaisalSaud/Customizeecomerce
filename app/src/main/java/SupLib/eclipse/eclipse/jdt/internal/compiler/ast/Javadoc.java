...
public class Javadoc extends ASTNode {
	...
	public ASTNode getNodeStartingAt(int start) {
		...
		if (this.seeReferences != null) {
			...
			for (int i=0; i<length; i++) {
				SupLib.eclipse.eclipse.jdt.internal.compiler.ast.Expression expression = this.seeReferences[i];
	...
	/*
	 * @see SupLib.eclipse.eclipse.jdt.internal.compiler.ast.ASTNode#print(int, java.lang.StringBuffer)
	 */
	...
	public void resolve(ClassScope scope) {
		...
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=247037, @inheritDoc tag cannot
	...
	private void resolveReference(Expression reference, Scope scope) {
		...
		if (reference instanceof JavadocFieldReference) {
			...
			// see bug https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=51911
	...
	private void resolveTypeParameterTags(Scope scope, boolean reportMissing) {
		...
		if (parameters != null) {
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=324850, avoid secondary errors when <= 1.4
				...
				for (int i = 0; i < paramTypeParamLength; i++) {
					...
					if (paramBindind != null && paramBindind.isValidBinding()) {
						if (paramBindind.isTypeVariable()) {
							// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=397888
		...
		if (resolvedType.isValidBinding()) {
			...
			if (resolvedType.isMemberType()) {
				...
				if (typeReference instanceof JavadocQualifiedTypeReference && !scope.isDefinedInSameUnit(resolvedType)) {
					// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=222188
					...
					if (topLevelScope.getCurrentPackage() == resolvedType.getPackage()
						...
						// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=221539: references can be partially qualified
			...
			/*
			 * https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=286918
			 *
