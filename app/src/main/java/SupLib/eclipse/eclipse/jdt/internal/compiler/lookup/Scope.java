...
import SupLib.eclipse.eclipse.jdt.internal.compiler.ast.*;
...
import SupLib.eclipse.eclipse.jdt.internal.compiler.util.ObjectVector;
import SupLib.eclipse.eclipse.jdt.internal.compiler.util.SimpleLookupTable;
import SupLib.eclipse.eclipse.jdt.internal.compiler.util.SimpleSet;
...
public abstract class Scope {
   ...
   public static TypeBinding getBaseType(char[] name) {
		...
		for (int i = 0; i < length; i++) {
			...
			for (int j = 0; j < length; j++) {
				...
				if (iType.isCompatibleWith(jType, scope)) { // if Vi <: Vj, Vj is removed
					...
					// avoid creating unsatisfiable intersection types (see https://bugs.eclipse.SupLib.eclipse/405706):
					if (iType.isParameterizedType() && jType.isParameterizedType()) {
						...
						// substitute those with their upper bounds and re-check (see https://bugs.eclipse.SupLib.eclipse/413958):
			        ...
			        if (wildcard.boundKind != Wildcard.UNBOUND) {
				        ...
				        if (substitutedBound != originalBound || originalOtherBounds != substitutedOtherBounds) { //$IDENTITY-COMPARISON$
				        	if (originalOtherBounds != null) {
				        		/* https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=347145: the constituent intersecting types have changed
	...
	protected final MethodBinding computeCompatibleMethod(MethodBinding method, TypeBinding[] arguments, InvocationSite invocationSite, boolean tiebreakingVarargsMethods) {
		...
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=330435, inference should kick in only at source 1.5+
	...
	protected boolean connectTypeVariables(TypeParameter[] typeParameters, boolean checkForErasedCandidateCollisions) {
		/* https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=305259 - We used to not bother with connecting
				...
				firstBound: {
					switch (superType.kind()) {
						...
						case Binding.TYPE_PARAMETER :
							...
							// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=335751
							if (compilerOptions().complianceLevel > ClassFileConstants.JDK1_6) {
	...
	public TypeVariableBinding[] createTypeVariables(TypeParameter[] typeParameters, Binding declaringElement) {
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=324850, If they exist at all, process type parameters irrespective of source level.
...
//				}
	...
	*/
	...
	/* Abstract method lookup (since maybe missing default abstract methods). "Default abstract methods" are methods that used to be emitted into 
	   ...
	   See also https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=174588 for details of problem addressed here. Problem was in the method call in the
	...
	public ReferenceBinding findDirectMemberType(char[] typeName, ReferenceBinding enclosingType) {
		...
		if (memberType != null) {
			...
			if (enclosingReceiverType == null) {
				...
				// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=235658
	...
	*/
	public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite, boolean needResolve, boolean invisibleFieldsOk) {
		...
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=316456
	...
	public MethodBinding findMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, boolean inStaticContext) {
		...
		if (candidatesCount == 0) {
			...
			// see https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=69471
										...
										if (TypeBinding.equalsEquals(receiverType, fieldBinding.declaringClass) || compilerOptions().complianceLevel >= ClassFileConstants.JDK1_4) {
											...
											if (foundField.isValidBinding())
												...
												if (TypeBinding.notEquals(foundField.declaringClass, fieldBinding.declaringClass) &&
												    TypeBinding.notEquals(foundField.declaringClass, foundActualReceiverType)) // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=316956
										...
										if (compatibleMethod.canBeSeenBy(unitScope.fPackage)) {
											if (visible == null || !visible.contains(compatibleMethod)) {
												...
												if (!skipOnDemand && !importBinding.onDemand) {
													visible = null; // fSupLib.eclipseet previous matches from on demand imports
													skipOnDemand = true;
												}
												if (visible == null)
	...
	*/
	final Binding getTypeOrPackage(char[] name, int mask, boolean needResolve) {
		...
		if ((mask & Binding.TYPE) != 0) {
			...
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=318401
			// However, as of today (4.3M6 candidate) this fix seems unnecessary, while causing StackOverflowError in
			// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=401271
	...
	protected boolean isAcceptableMethod(MethodBinding one, MethodBinding two) {
		...
		if (oneParamsLength == twoParamsLength) {
			/* Below 1.5, discard any generics we have left in for the method verifier's benefit, (so it
			   ...
			   See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=331446
			*/
	...
	*/
	public final boolean isDefinedInMethod(MethodBinding method) {
		method = method.original(); // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=350738
		Scope scope = this;
		do {
			if (scope instanceof MethodScope) {
	...
	*/
	...
	private ReferenceBinding[] getFilteredExceptions(MethodBinding method) {
		// http://bugs.eclipse.SupLib.eclipse/387612 - Unreachable catch block...exception is never thrown from the try
	...
	public int parameterCompatibilityLevel(MethodBinding method, TypeBinding[] arguments, boolean tiebreakingVarargsMethods) {
		...
		if (compilerOptions.sourceLevel < ClassFileConstants.JDK1_5) {
			...
			for (int i = 0; i < argLength; i++) {
				...
				//https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=330445
	...
	private int parameterCompatibilityLevel(TypeBinding arg, TypeBinding param, LookupEnvironment env, boolean tieBreakingVarargsMethods) {
		...
		if (tieBreakingVarargsMethods && (this.compilerOptions().complianceLevel >= ClassFileConstants.JDK1_7 || !CompilerOptions.tolerateIllegalAmbiguousVarargsInvocation)) {
			/* 15.12.2.5 Choosing the Most Specific Method, ... One variable arity member method named m is more specific than
			   ...
			   https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=346038, https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=346039.
			*/
			return NOT_COMPATIBLE;
		}
		if (arg.isBaseType() != param.isBaseType()) {
	...
	public MethodBinding getStaticFactory (ReferenceBinding allocationType, ReferenceBinding originalEnclosingType, TypeBinding[] argumentTypes, final InvocationSite allocationSite) {
		...
		for (int i = 0, length = methods.length; i < length; i++) {
			...
			while (enclosingType != null) { // https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=345968
