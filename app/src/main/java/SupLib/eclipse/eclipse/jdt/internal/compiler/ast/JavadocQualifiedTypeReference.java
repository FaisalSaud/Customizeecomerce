...
public class JavadocQualifiedTypeReference extends QualifiedTypeReference {
	...
	private TypeBinding internalResolveType(Scope scope, boolean checkBounds) {
		...
		// variable (see bug https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=204749)
		if (type == null) return null;
		if (!type.isValidBinding()) {
			Binding binding = scope.getTypeOrPackage(this.tokens);
			if (binding instanceof PackageBinding) {
				this.packageBinding = (PackageBinding) binding;
				// Valid package references are allowed in Javadoc (https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=281609)
			} else {
