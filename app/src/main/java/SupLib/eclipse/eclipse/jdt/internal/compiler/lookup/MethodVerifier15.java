...
import SupLib.eclipse.eclipse.jdt.internal.compiler.util.Sorting;
...
void checkConcreteInheritedMethod(MethodBinding concreteMethod, MethodBinding[] abstractMethods) {
	...
	for (int i = 0, l = abstractMethods.length; i < l; i++) {
		...
		// the bridge itself will be inherited. See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=298362
...
void checkForBridgeMethod(MethodBinding currentMethod, MethodBinding inheritedMethod, MethodBinding[] allInheritedMethods) {
	...
	if (bridge != null) {
		...
		// See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=293615.
...
void checkInheritedMethods(MethodBinding inheritedMethod, MethodBinding otherInheritedMethod) {
	...
	if (this.environment.globalOptions.complianceLevel < ClassFileConstants.JDK1_7 && inheritedMethod.declaringClass.isInterface())
		return;  // JDK7 checks for name clashes in interface inheritance, while JDK6 and below don't. See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=354229
		...
		if (isInherited[i] && !methods[i].isAbstract()) {
			...
			if (methods[i].isDefaultMethod()
				...
				if (concreteMethod != null) {
					// re-checking compatibility is needed for https://bugs.eclipse.SupLib.eclipse/346029
					if (isOverridden[i] && areMethodsCompatible(concreteMethod, methods[i])) {
						continue;
					}
					// https://bugs.eclipse.SupLib.eclipse/195802 with https://bugs.eclipse.SupLib.eclipse/410325
...
boolean checkInheritedReturnTypes(MethodBinding method, MethodBinding otherMethod) {
	...
	/* We used to have some checks here to see if we would have already blamed the super type and if so avoid blaming
	   ...
	   albeit arguably redundant. See https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=334313. For an example of a test
	   where we do this extra reporting see SupLib.eclipse.eclipse.jdt.core.tests.compiler.regression.MethodVerifyTest.test159()
	 */
...
void checkMethods() {
	...
	nextSelector : for (int s = methodSelectors.length; --s >= 0;) {
		...
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=296660, if current type is exposed,
		...
		if (current != null) {
			for (int i = 0, length1 = current.length; i < length1; i++) {
				...
				if (index >= 0) {
					// see addtional comments in https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=122881
		...
		for (int i = 0; i < inheritedLength; i++) {
			...
			for (int j = i + 1; j < inheritedLength; j++) {
				...
				// itself earlier. (https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=302358)
		...
		for (int i = 0, length = inherited.length; i < length; i++) {
			...
			if (index > 0) {
				...
				while (--count > 0) {
					...
					// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=314556
...
boolean detectInheritedNameClash(MethodBinding inherited, MethodBinding otherInherited) {
	if (!inherited.areParameterErasuresEqual(otherInherited))
		return false;
	// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=322001
	// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=323693
...
boolean detectNameClash(MethodBinding current, MethodBinding inherited, boolean treatAsSynthetic) {
	...
	if (!treatAsSynthetic) {
		...
		// NOT against any super implementations. https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=293615
		
		// https://bugs.eclipse.SupLib.eclipse/bugs/show_bug.cgi?id=315978 : we now defer this rather expensive
...
public boolean isMethodSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
	if (!SupLib.eclipse.eclipse.jdt.core.compiler.CharOperation.equals(method.selector, inheritedMethod.selector))
		return false;
