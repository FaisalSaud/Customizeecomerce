...
public class BuildToolInfo {
    ...
    public BuildToolInfo(@NonNull FullRevision revision, @NonNull File path) {
        ...
        add(PathId.AAPT, FN_AAPT);
        add(PathId.AIDL, FN_AIDL);
        add(PathId.DX, FN_DX);
        add(PathId.DX_JAR, FD_LIB + File.separator + FN_DX_JAR);
        add(PathId.LLVM_RS_CC, FN_RENDERSCRIPT);
        add(PathId.ANDROID_RS, OS_FRAMEWORK_RS);
        add(PathId.ANDROID_RS_CLANG, OS_FRAMEWORK_RS_CLANG);
        add(PathId.DEXDUMP, FN_DEXDUMP);
        add(PathId.BCC_COMPAT, FN_BCC_COMPAT);
        add(PathId.LD_ARM, FN_LD_ARM);
        add(PathId.LD_X86, FN_LD_X86);
        add(PathId.LD_MIPS, FN_LD_MIPS);
        add(PathId.ZIP_ALIGN, FN_ZIPALIGN);
        add(PathId.JACK, FN_JACK);
        add(PathId.JILL, FN_JILL);
        add(PathId.SPLIT_SELECT, FN_SPLIT_SELECT);
    }
